...
def send_draw_detailed_rectangle():
   ...
   sftp_client=ssh.open_sftp() # SFTP clients allow us to perform secure file tranfer
   sftp_client.put('detailed_rectangle.pkl','/home/pi/Documents/FinalProject/detailed_rectangle.pkl') # NOTE: Need to add this folder to the Raspberry Pi
   sftp_client.close()

   # xhost + && export DISPLAY='10.27.148.134:0.0' && 

   ssh.exec_command("sudo killall pigpiod")
   stdin, stdout, stderr = ssh.exec_command("cd /home/pi/Documents/FinalProject/ && source /home/pi/miniconda3/bin/activate robotics3_env && python -V && sudo pigpiod && python -m DrawDetailedRectangle", get_pty=True)
   for line in iter(stdout.readline, ""):
      print(line, end="")
   print('finished.')
